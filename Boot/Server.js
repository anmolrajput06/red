	...
	Sys.Io = require('socket.io')(Sys.Server,{'pingTimeout' :Sys.Config.Socket.pingTimeout,'pingInterval' : Sys.Config.Socket.pingInterval});
	 ...
	 Sys.Io.on('connection',async function(socket) {
		...
		socket.use((packet, next) => {
			const eventName = packet[0]; // Get event name
			const allowedEvents = ["login","register"]; // Events that skip auth
	
			if (allowedEvents.includes(eventName)) {
				return next(); // Skip auth for these events
			}else{
				const data = JSON.parse(packet[1]); // Get event data
				// Require auth for other events
				if (data.authToken!=undefined && data.authToken!="") {
					jwt.verify(data.authToken, Sys.Config.Database.JWT_KEY, async function(err, decoded) {
						if (err) {
							...
							let player = await Sys.Game.Common.Services.PlayerServices.findOneUser({_id: mongoose.Types.ObjectId(decoded.id),authToken:data.authToken});
